...
const ReviewService = {
    async createReview(userId: string, data: Partial<IReview>): Promise<IReview> {
        ...
        // TODO: Check if user has purchased the product for isVerifiedPurchase
    ...
    async getProductReviews(productId: string, page = 1, limit = 10): Promise<{ data: IReview[]; total: number; avgRating: number }> {
        ...
        return { data: reviews, total, avgRating: Math.round(avgRating * 10) / 10 };
    },
...
const ReviewController = {
    createReview: catchAsync(async (req: Request, res: Response) => {
        ...
        sendResponse(res, { statusCode: 201, success: true, message: 'Review submitted for approval', data: review });
    }),
    ...
    getProductReviews: catchAsync(async (req: Request, res: Response) => {
        ...
        sendResponse(res, {
            ...
            data: { reviews: result.data, avgRating: result.avgRating },
        });
    }),
