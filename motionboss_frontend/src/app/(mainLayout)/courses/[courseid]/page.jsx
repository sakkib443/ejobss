...
import {
  FaStar, FaUsers, FaGraduationCap, FaArrowRight,
...
const SingleCourse = () => {
  ...
  useEffect(() => {
    if (courses && courses.length > 0) {
      const foundCourse = courses.find((c) => c._id === courseId || c.id === courseId);
      if (foundCourse) {
        setCurrentCourse(foundCourse);
        setPopularCourses(courses.filter((c) => (c._id !== courseId && c.id !== courseId)).slice(0, 3));
  ...
  if (!currentCourse && !loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center">
        <h2 className="text-2xl font-black mb-4">Course Not Found</h2>
        <Link href="/courses" className="px-6 py-2 bg-[#41bfb8] text-white rounded-xl">Back to Courses</Link>
      </div>
    );
  }
  ...
  const thumbnail = currentCourse.thumbnail || currentCourse.image;
  const price = currentCourse.price || (parseInt(currentCourse.fee?.replace(/[^\d]/g, '') || 0));
  const discountPrice = currentCourse.discountPrice;
  const type = currentCourse.courseType || currentCourse.type;
  const rating = currentCourse.averageRating || currentCourse.rating || 5;
  const enrollments = currentCourse.totalEnrollments || currentCourse.totalStudentsEnroll || 0;
          ...
          <div className="max-w-4xl space-y-6">
            ...
            >
              <span className={`px-4 py-1.5 bg-gradient-to-r from-teal-500 to-cyan-500 rounded-full text-white text-xs font-bold uppercase tracking-widest work shadow-lg shadow-teal-200/50 ${bengaliClass}`}>
                {t("courseDetails.bestSeller") || "PREMIUM COURSE"}
              </span>
              <span className={`px-4 py-1.5 bg-white/80 backdrop-blur-sm border border-gray-200 rounded-full text-gray-700 text-xs font-bold uppercase tracking-widest work shadow-sm ${bengaliClass}`}>
                {type} {t("courseDetails.learning") || "Learning"}
              </span>
            </motion.div>
            ...
            >
              {currentCourse.courseOverview || currentCourse.description || currentCourse.details?.substring(0, 160) + "..."}
            </motion.p>
            ...
            >
              <div className="flex items-center gap-3 bg-white/70 backdrop-blur-sm px-4 py-2 rounded-2xl shadow-sm border border-gray-100">
                ...
                <span className="font-bold outfit text-lg text-gray-900">{rating}</span>
              </div>
              <div className="flex items-center gap-3 bg-white/70 backdrop-blur-sm px-4 py-2 rounded-2xl shadow-sm border border-gray-100">
                ...
                <span className={`text-gray-700 font-medium work ${bengaliClass}`}>{enrollments}+ {t("courseDetails.studentsEnrolled") || "Students"}</span>
              </div>
            </motion.div>
            ...
            >
              <div className="flex items-center gap-2 group cursor-pointer bg-white/60 backdrop-blur-sm px-4 py-2 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all">
                <span className={`text-gray-500 work ${bengaliClass}`}>{t("courseDetails.instructedBy") || "Instructed by"}</span>
                <span className="text-teal-600 font-bold outfit underline decoration-teal-400/30 underline-offset-4 group-hover:decoration-teal-600 transition-all">
                  {instructor?.name || t("courseDetails.seniorExpert") || "Industry Expert"}
                </span>
              ...
              <div className="flex border-b border-gray-100 bg-gray-50/50 p-2">
                {[
                  ...
                  >
                    ...
                    <span className="hidden sm:inline">{tab.label || tab.id}</span>
                  </button>
                ))}
              </div>
                    ...
                    >
                      <div className="space-y-6">
                        <h2 className={`text-3xl font-bold outfit text-gray-900 flex items-center gap-3 ${bengaliClass}`}>
                          ...
                          {t("courseDetails.courseDetails") || "Course Details"}
                        </h2>
                        <div className="text-gray-600 work text-lg leading-relaxed text-justify whitespace-pre-line">
                          {currentCourse.details || currentCourse.description}
                        </div>
                      </div>
                    </motion.div>
                  )}
                  ...
                  {activeTab === "curriculum" && (
                    ...
                    >
                      <div className="space-y-6">
                        <h2 className={`text-3xl font-bold outfit text-gray-900 flex items-center gap-3 ${bengaliClass}`}>
                          <span className="w-2 h-8 bg-teal-500 rounded-full"></span>
                          {t("courseDetails.learningModules") || "Curriculum"}
                        </h2>
                          ...
                          {currentCourse.curriculum?.map((item, idx) => (
                            <div key={idx} className="flex items-center justify-between p-5 bg-[#F9FBFC] border border-gray-100 rounded-2xl hover:bg-white hover:shadow-md transition-all group">
                              <div className="flex items-center gap-4">
                                ...
                                <span className="font-bold text-gray-800 work">{typeof item === 'string' ? item : item.title}</span>
                              </div>
                              <MdPlayCircleOutline className="text-gray-300 group-hover:text-teal-500 text-2xl transition-colors cursor-pointer" />
                            </div>
                          ))}
                          {!currentCourse.curriculum?.length && <p>Curriculum details coming soon.</p>}
                        </div>
                      </div>
                    </motion.div>
                  )}
                    ...
                    >
                      {instructor ? (
                            ...
                            <div className="flex-1 space-y-4">
                              ...
                              <div className="flex gap-4">
                                <div className={`flex items-center gap-2 px-4 py-2 bg-gray-50 rounded-xl ${bengaliClass}`}>
                                  ...
                                  <span className="font-bold outfit">4.9/5 {t("courseDetails.ratings") || "Rating"}</span>
                                </div>
                              </div>
                        ...
                        <div className="text-center py-20 bg-gray-50 rounded-[3rem] border-2 border-dashed border-gray-200">
                          ...
                          <h4 className={`text-2xl font-bold text-gray-700 outfit ${bengaliClass}`}>{t("courseDetails.mentorNotAssigned") || "Instructor Coming Soon"}</h4>
                        </div>
                      )}
                    </motion.div>
                  )}
          ...
          <div className="lg:col-span-4 sticky top-28 space-y-8">
            ...
            <div className="bg-white rounded-[2.5rem] shadow-2xl border border-teal-50 overflow-hidden">
              <div className="relative aspect-video group cursor-pointer overflow-hidden">
                <img src={thumbnail} alt={title} className="w-full h-full object-cover transition-transform group-hover:scale-110 duration-700" />
              ...
              <div className="p-10 space-y-8">
                <div className="space-y-2">
                  <div className="flex items-center gap-3">
                    <span className="text-4xl font-extrabold text-[#0A1D1C] outfit leading-none">৳{price.toLocaleString()}</span>
                    {discountPrice && (
                      <span className="line-through text-gray-400 font-bold work text-lg">৳{price + 2000}</span>
                    )}
                  </div>
                  <p className={`text-teal-600 font-bold work text-sm uppercase tracking-widest ${bengaliClass}`}>{t("courseDetails.earlyBird") || "Special Enrollment Price"}</p>
                </div>

                <div className="space-y-4">
                  <button
                    onClick={() => router.push(`/checkout?courseId=${courseId}`)}
                    className={`w-full py-5 bg-[#43c3bc] hover:bg-[#38a89d] text-white font-bold rounded-2xl shadow-xl shadow-teal-50 transform transition-all active:scale-95 flex items-center justify-center gap-3 ${bengaliClass}`}
                  >
                    {t("courseDetails.enrollCourse") || "Purchase Now"}
                    <FaArrowRight className="text-sm" />
                  </button>
                </div>

                <div className="space-y-6 pt-4 border-t border-gray-50">
                  <h5 className={`font-bold outfit text-gray-900 ${bengaliClass}`}>{t("courseDetails.whatIncluded") || "Course Features"}</h5>
                  <ul className="space-y-4">
                    <li className={`flex items-center gap-3 text-gray-600 work text-sm font-medium ${bengaliClass}`}>
                      ...
                      {currentCourse.totalLessons || currentCourse.lectures || 120}+ {t("courseDetails.videoLessons") || "Lessons"}
                    </li>
                    <li className={`flex items-center gap-3 text-gray-600 work text-sm font-medium ${bengaliClass}`}>
                      <LuClock className="text-teal-500 text-lg" />
                      {currentCourse.totalDuration || "3 Months"} {t("courseDetails.duration") || "Duration"}
                    </li>
                    <li className={`flex items-center gap-3 text-gray-600 work text-sm font-medium ${bengaliClass}`}>
                      <LuTrophy className="text-teal-500 text-lg" />
                      {t("courseDetails.certificate") || "Industry Certificate"}
                    </li>
                  </ul>
                </div>
              </div>
            ...
            <div className="bg-[#F8FAFB] rounded-[2.5rem] p-8 space-y-8 border border-gray-100">
              <h3 className={`text-xl font-bold outfit text-gray-900 ${bengaliClass}`}>{t("courseDetails.recommendedCourses") || "Popular Courses"}</h3>
              <div className="space-y-6">
                {popularCourses.map(item => (
                  <Link href={`/courses/${item._id || item.id}`} key={item._id || item.id} className="flex gap-4 group">
                    <div className="w-20 h-20 rounded-2xl overflow-hidden flex-shrink-0 bg-white shadow-sm border border-slate-100">
                      <img src={item.thumbnail || item.image || "/placeholder-course.jpg"} alt={item.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform" />
                    </div>
                    <div className="flex-1 space-y-1">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <span className="text-teal-600 font-bold work text-xs">৳{(item.price || parseInt(item.fee?.replace(/[^\d]/g, '') || 0)).toLocaleString()}</span>
                      </div>
                    </div>
                  </Link>
                ))}
