        ...
        return (
            <div className="group relative inline-block">
                ...
                <button
                    ...
                    className="absolute -right-8 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity p-1.5 bg-[#41bfb8] text-white rounded-lg shadow-lg hover:bg-[#2dd4bf] z-50"
                    title="Edit this text"
                >
                    <FiEdit2 size={14} />
                </button>
                {/* Highlight border on hover */}
                <div className="absolute inset-0 border-2 border-dashed border-transparent group-hover:border-[#41bfb8]/50 rounded pointer-events-none -m-1" />
            </div>
        );
    }
        ...
        <div className="relative">
            {isMultiLine ? (
                <textarea
                    ...
                    className={`w-full p-2 border-2 border-[#41bfb8] rounded-lg outline-none bg-white text-gray-800 resize-none ${className}`}
                    rows={4}
                    disabled={loading}
                />
            ) : (
                <input
                    ...
                    onChange={(e) => setEditValue(e.target.value)}
                    onKeyDown={handleKeyDown}
                    className={`w-full p-2 border-2 border-[#41bfb8] rounded-lg outline-none bg-white text-gray-800 ${className}`}
                    disabled={loading}
                />
            )}
            <div className="absolute -right-20 top-1/2 -translate-y-1/2 flex gap-1">
                <button
                    onClick={handleSave}
                    disabled={loading}
                    className="p-1.5 bg-[#41bfb8] text-white rounded-lg shadow-lg hover:bg-[#2dd4bf] disabled:opacity-50"
                    title="Save"
                >
                    <FiCheck size={14} />
                </button>
