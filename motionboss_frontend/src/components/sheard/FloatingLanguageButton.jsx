    ...
    <>
      <div className="fixed right-0 top-1/2 -translate-y-1/2 z-50 flex items-center">
        ...
        <button
          ...
          className={`bg-gradient-to-b from-[#41bfb8] to-[#38a89d] text-white px-2 py-6 rounded-l-md shadow-md hover:shadow-lg hover:px-3 transition-all duration-300 flex items-center justify-center ${isOpen ? 'px-0 w-0 overflow-hidden opacity-0' : 'px-2 opacity-100'}`}
        >
        ...
        <div className={`overflow-hidden transition-all duration-500 ease-in-out ${isOpen ? 'max-w-[200px] opacity-100' : 'max-w-0 opacity-0'}`}>
          <div className="flex items-center">
            ...
            <Link href="/language-program">
              <div
                className="animated-gradient-btn text-white py-3 px-5 shadow-lg hover:shadow-xl hover:shadow-[#41bfb8]/40 transition-all duration-300 hover:px-6 cursor-pointer rounded-l-3xl"
                style={{ writingMode: 'vertical-rl', textOrientation: 'mixed' }}
              >
                <span className={`text-base font-bold tracking-wider uppercase whitespace-nowrap ${fontClass}`}>
                  {language === "bn" ? "ভাষা প্রোগ্রাম" : "Language Program"}
                </span>
              </div>
            </Link>
          </div>
